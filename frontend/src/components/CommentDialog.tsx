...
import { CREATE_COMMENT, CREATE_LIKE_COMMENT, CREATE_NOTIF, CREATE_REPLY_COMMENT, GET_ALL_POST, GET_ALL_POST_NORES, GET_POST, INSERT_POST } from '../query/query';
...
const CommentDialog: React.FC<CommentDialogProps> = ({ onClose, open, disableBackdropClick, forum, userId, refetchForum }) => {
  if (!open) return null;

  const {data : newestForum, refetch: refetchNewest} = useQuery(GET_POST, {
    variables : {
      id : forum?.posts.id
    }
  })
  ...
  const handlePost = async () => {
    ...
    if(forum.posts.userID !== userId){
      await createNotif({
        variables : {
          inputNotif : {
            userID: forum.posts.userID,
            notif: "Someone has commented on your post",
            user2ID: userId,
          }
        }
      })
  ...
  useEffect(() => {
    ...
    if(newestForum)
      setSelectedForum(newestForum.getPost)
  }, [newestForum])
  ...
  return (
    selectedForum && (<div className="custom-dialog-overlay" onClick={handleClickOutside}>
      <div className="custom-dialog-content">
        ...
        <h3 style={{fontSize:"20px"}}> {selectedForum?.posts?.userName + "\'s Post"} </h3>
